...
function initializeAnimations() {
  // Welcome Section: Spiral Gate Animations
...
function animateWelcomeSection() {
  ...
  // Animate spiral rings with different speeds and directions
  const rings = document.querySelectorAll(".spiral-ring");
  rings.forEach((ring, index) => {
    const speed = 8 + index * 2;
    const direction = index % 2 === 0 ? 1 : -1;

    gsap.timeline({ repeat: -1 }).to(ring, {
  ...
  // Animate code lines flowing into the spiral
  ...
  const welcomeTitle = document.querySelector(".welcome-title .text-animation");
  const welcomeDescription = document.querySelector(
    ".welcome-description .text-animation"
  );
        ...
        {},
        {
          ...
          onRepeat: function () {
            const currentLength = Math.floor(
              this.progress() * titleText.length
            );
            welcomeTitle.textContent = titleText.substring(
              0,
              currentLength + 1
            );
          },
  ...
  if (welcomeDescription) {
    const descText = welcomeDescription.textContent;
    welcomeDescription.textContent = "";

    gsap
      .timeline({ delay: 1.5 })
      .to(welcomeDescription, {
        opacity: 1,
        duration: 0.1,
      })
      .to(
        ...
        {
          duration: 0.03,
          repeat: descText.length - 1,
          onRepeat: function () {
            const currentLength = Math.floor(this.progress() * descText.length);
            welcomeDescription.textContent = descText.substring(
              0,
              currentLength + 1
            );
          },
...
function animateScadaScene() {
  ...
  signalWaves.forEach((wave, index) => {
    gsap.timeline({ repeat: -1, repeatDelay: 1 }).to(wave, {
      scale: 3,
      opacity: 0,
      duration: 2,
      ease: "power2.out",
